    ...
    for i in range(1, len(subset)):
        ...
        if len(subset[i]) == 1:
            ...
            for n in range(1, len(subset[i])):
                x_i = pd.concat([x_i, dfx[subset[i][n]]], axis = 0)
...
def FeaIndepenVal(x_train_val, y_train, initial, model, Tn, n, time_columns):
    result = pd.DataFrame(initial)
    for col in x_train_val.columns[:n]:
        x_train_val.drop(columns = col, inplace = True)
        model_val = model.fit(x_train_val, y_train)
        Value = ShapleyValues(model_val, x_train_val, Tn)
        result[col] = Value
        
    result = result.T
    result.columns = time_columns
...
def PlotVal (result):
    sns.set(context='paper', style='ticks', font_scale=1.95)
    plt.figure(figsize = (5,5))

    plt.plot(result, label = result.columns)


    plt.ylabel('ShapTime Values', fontsize=16)
